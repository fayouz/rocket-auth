 ...
 * (rocket-core, Rocket\Core\Suite\SuiteApps): the enabled clients that have an address, and the interface of Rocket Auth
 * itself, where users manage their account ("Mon compte").
 */
final class SuiteAppsController extends AbstractController
{
    #[Route('/api/suite/apps', name: 'api_suite_apps', methods: ['GET'])]
    public function __invoke(OAuthClientRepository $clients, #[Autowire(env: 'FRONTEND_URL')] string $frontendUrl): JsonResponse
    {
        $apps = [];
        ...
        $response = $this->json(['apps' => $apps, 'account' => rtrim($frontendUrl, '/')]);
